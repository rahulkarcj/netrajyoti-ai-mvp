...
export default function App() {
  ...
  if (screen === 'routineResult') return <Page back={() => setScreen('routineDetails')}><Step label="Step 6" bn="চোখ পরীক্ষা করানোর পরামর্শ" en="Your recommended next step" /><RouteResultSummary outcome="ROUTINE" choices={selectedShareSymptoms} /><RouteExplanation summary={aiSummary} loading={aiLoading} outcome="ROUTINE" /><div className="routine-result-card"><b>কয়েক দিনের মধ্যে ভিশন সেন্টার বা চোখের ক্লিনিকে যাওয়ার পরিকল্পনা করুন।</b><small>Plan a visit to a vision centre or eye clinic in the next few days.</small></div><div className="routine-nearby"><div className="routine-nearby-heading"><b>কাছাকাছি চোখের সেবার বিকল্প</b><small>Nearby eye-care options</small></div><p>{district && block ? `${district} · ${block} এলাকার কাছাকাছি চোখের সেবার তালিকা` : 'নির্বাচিত এলাকার কাছাকাছি চোখের সেবার তালিকা'}<small>Nearby eye-care options for the selected area</small></p>{routinePreviewFacilities.map(facility => <div className="routine-preview-item" key={facility.en}><b>{facility.name}</b><small>{facility.en}<br />{facility.type}</small></div>)}<span className="routine-preview-note">সেবা নেওয়ার আগে সময়, ঠিকানা এবং সেবা পাওয়া যাবে কি না নিশ্চিত করুন।<small>Confirm hours, address, and availability before you travel.</small></span></div><Primary onClick={() => setScreen('serviceDirection')}>পরিষেবার দিকনির্দেশনা দেখুন<small>View service direction</small></Primary></Page>;
  ...
  if (screen === 'human') return <Page back={() => setScreen('decision')}><Step label="5C" bn="একজন স্বাস্থ্যকর্মীর সঙ্গে কথা বলুন" en="Get help from a person near you" /><RouteResultSummary outcome="HUMAN_SUPPORT" choices={selectedShareSymptoms} /><RouteExplanation summary={aiSummary} loading={aiLoading} outcome="HUMAN_SUPPORT" /><div className="human-support-card"><span className="human-support-label">মানব সহায়তা<small>Human support</small></span><b>আপনাকে একা সিদ্ধান্ত নিতে হবে না। কাছের ASHA কর্মী, PHC বা চোখের সেবার পথ খুঁজে নিন।</b><small>You do not have to decide alone. Find a nearby ASHA worker, PHC, or eye-care route.</small></div><Primary onClick={() => setScreen('humanOptions')}>কাছের সহায়তা খুঁজুন<small>Find nearby support</small></Primary><Secondary onClick={restart}>আবার শুরু করুন<small>Start again</small></Secondary></Page>;
...
function RouteResultSummary({ outcome, choices }: { outcome: Outcome; choices: Choice[] }) {
  const labels = outcome === 'URGENT'
    ? { bn: 'নির্বাচিত নিরাপদ পথ: জরুরি চোখের চিকিৎসা', en: 'Selected safe route: Urgent eye care', className: 'urgent' }
    : outcome === 'ROUTINE'
      ? { bn: 'নির্বাচিত নিরাপদ পথ: নিয়মিত চোখ পরীক্ষা', en: 'Selected safe route: Routine eye examination', className: 'routine' }
      : { bn: 'নির্বাচিত নিরাপদ পথ: মানব সহায়তা', en: 'Selected safe route: Human support', className: 'human' };
  return <section className={`route-result-summary ${labels.className}`} aria-label={labels.en}>
    <div className="route-result-label"><b>{labels.bn}</b><small>{labels.en}</small></div>
    <SymptomSummary choices={choices} />
  </section>;
}
