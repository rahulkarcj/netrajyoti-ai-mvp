...
function RouteExplanation({ summary, loading, outcome }: { summary: CaregiverSummary | null; loading: boolean; outcome: Outcome }) {
  ...
  const routeEnglish = outcome === 'URGENT'
    ? 'Based on the information shared, urgent eye care is important today. Do not delay. If possible, take a trusted family member with you.'
    : outcome === 'ROUTINE'
      ? 'Based on your selected symptoms, when no urgent warning signs are present, arrange an eye examination at a nearby vision centre or eye clinic in the next few days. Confirm hours before you travel.'
      : 'Please seek support from a health worker or eye-care service for the next step.';
  return <div className="rag-source-note"><b>নিরাপদ নির্দেশনা</b><p>{summary.summaryBn}</p><small>{routeEnglish}</small>{summary.sources.length > 0 && <div className="rag-reference"><b>তথ্যসূত্র</b><small>Reference source</small><ul>{summary.sources.map(source => <li key={source.id}>{source.title} <span>· Source accessed {source.reviewedOn}</span></li>)}</ul></div>}</div>;
}
